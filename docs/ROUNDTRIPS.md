...
| Azure transport isolation | Unchanged (small operations: 1 -> 1) | 0 additional; independent control/bulk transports share one auth cache; staged PUT and compose budgets unchanged | `azure.rs::AzureClient`, `data_client` |
| Event Grid subscription validation (native POST / CloudEvents OPTIONS) | 0 -> 0 bucket rounds | 0; read auth and bridge-enabled checks only; notification catch-up retains its existing WAL/cursor work | `bridge.rs::http_notify`, `http_notify_options` |
